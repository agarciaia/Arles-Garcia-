...
export default async function handler(req, res) {
  try {
    ...
    if (!/^[a-z0-9._-]{3,40}$/.test(username) || password.length < 6)
      throw invalid();
    ...
    const mapping = await db.doc(`workshopUsernames/${username}`).get();
    if (!mapping.exists) throw invalid();
    const { uid, emailInternal } = mapping.data();
    ...
    if (verified.localId !== uid) throw invalid();
    const accountRef = db.doc(`accounts/${uid}`);
    const account = (await accountRef.get()).data();
    if (!account || account.status === "suspended") {
      const error = new Error(
        "La cuenta no está disponible. Contacta al administrador.",
      );
