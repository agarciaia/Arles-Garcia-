...
import { auth, deleteUser, googleProvider, reload, sendEmailVerification, sendPasswordResetEmail, signInWithEmailAndPassword, signInWithCustomToken, signInWithPopup, signOut } from '../firebase';
...
const Settings: React.FC<SettingsProps> = ({ user, settings, setSettings, services, setServices, costs, setCosts, quotes, setQuotes, syncStatus, account, canWrite }) => {
  ...
  const handleImportFullBackupJSON = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (event) => {
      try {
        ...
        const validSettings = data.settings && typeof data.settings === 'object' && typeof data.settings.companyName === 'string' && typeof data.settings.whatsappServiceTemplate === 'string' && typeof data.settings.whatsappQuoteTemplate === 'string';
  ...
  const exportServices = () => {
    const headers = ['ID Servicio', 'Estado', 'Fecha Ingreso', 'Fecha Término', 'Cliente', 'Teléfono', 'Vehículo', 'Patente', 'Motivo Ingreso', 'Detalle Trabajos', 'Costos Asociados', 'Monto Total', 'Monto Abono', 'Fecha Abono'];
    ...
    const rows = services.map((s) => {
      ...
      return [s.id, s.status === 'completed' ? 'Completado' : s.status === 'in-progress' ? 'En Proceso' : s.status === 'cancelled' ? 'Cancelado' : 'Pendiente', new Date(s.entryDate).toLocaleDateString(), endDate, s.clientName, s.phone, `${s.brand} ${s.model}`, s.plate, s.reason, laborDetails, expenseDetails, total, advanceAmount, advanceDate];
    });
  ...
  const exportCosts = () => {
    ...
    const rows = costs.map((c) => [c.id, new Date(c.date).toLocaleDateString(), c.description, categoryLabels[c.category] || c.category, c.amount]);

    downloadCSV(headers, rows, `gastos_taller_${new Date().toISOString().split('T')[0]}.csv`);
  };

  const exportQuotes = () => {
    const headers = ['ID Cotización', 'Fecha Emisión', 'Cliente', 'Teléfono', 'Vehículo', 'Detalle Items', 'Notas', 'Validez (Días)', 'Total ($)'];
    ...
    const rows = quotes.map((q) => {
      ...
      return [q.id, new Date(q.date).toLocaleDateString(), q.clientName, q.phone, q.vehicle, details, q.notes || '', q.validityDays, q.total];
    });
  ...
  const handleImportServices = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const newServices: Service[] = dataRows
        .map((row) => {
  ...
  const handleImportQuotes = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const newQuotes: Quote[] = dataRows
        .map((row) => {
  ...
  const VariableChip = ({ label, code }: { label: string; code: string }) => (
    <button onClick={() => setTempTemplateValue((prev) => prev + ' ' + code)} className="flex-shrink-0 inline-flex items-center gap-2 px-4 py-3 bg-slate-800 hover:bg-slate-700 rounded-xl text-sm font-medium text-slate-300 hover:text-white transition-all border border-slate-700 shadow-sm active:scale-95">
  ...
  const handleWorkshopAuth = async (e: React.FormEvent) => {
    e.preventDefault();
    if (isAuthLoading) return;
    setIsAuthLoading(true);
    setAuthError('');
    ...
    try {
      ...
      if (!response.ok || !data.token) throw new Error('Usuario o contraseña incorrectos.');
  ...
  const handleDeleteAccount = async () => {
    ...
    try {
      ...
      ['taller_services', 'taller_costs', 'taller_quotes', 'taller_settings', 'taller_owner_uid', 'service_draft'].forEach((key) => localStorage.removeItem(key));
      window.location.reload();
    } catch (error: any) {
      setAuthError(getAuthErrorMessage(error.code));
    } finally {
    ...
    return (
      <div className="space-y-6 pb-20 animate-fade-in">
        ...
        <div className="grid grid-cols-1 gap-4">
            ...
            {user ? (
              <>
                <div className="flex flex-col sm:flex-row items-center gap-4 bg-slate-900/50 p-4 rounded-xl border border-slate-700">
                  {user.photoURL ? <img src={user.photoURL} alt={user.email || 'User'} className="w-12 h-12 rounded-full border-2 border-blue-500/50" /> : <div className="w-12 h-12 rounded-full bg-blue-500/10 flex items-center justify-center text-blue-400 font-bold border-2 border-blue-500/50">{user.email?.charAt(0).toUpperCase()}</div>}
                  <div className="flex-1 text-center sm:text-left overflow-hidden">
                    <p className="text-white font-bold truncate">{account?.businessName || user.displayName || 'Usuario Activo'}</p>
                    <p className="text-slate-400 text-xs truncate italic">{account?.managed ? `@${account.username || 'taller'}` : user.email}</p>
                  ...
                  <div className="flex gap-2">
                    ...
                    <button onClick={() => signOut(auth)} disabled={isAuthLoading} className="px-3 py-2 bg-red-900/20 border border-red-500/30 text-red-500 rounded-lg text-xs font-medium hover:bg-red-500 hover:text-white transition-all disabled:opacity-50 disabled:cursor-not-allowed">
                      Cerrar Sesión
                    </button>
                  </div>
                </div>
                {!user.emailVerified && (
                  <div className="w-full mt-4 rounded-xl border border-amber-500/30 bg-amber-500/10 p-4">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 mt-3">
                      <button onClick={handleResendVerification} disabled={isAuthLoading} className="py-2 rounded-lg border border-amber-500/30 text-amber-100 text-sm font-bold">
                        Reenviar correo
                      </button>
                      <button onClick={handleCheckVerification} disabled={isAuthLoading} className="py-2 rounded-lg bg-amber-500 text-slate-950 text-sm font-bold">
                        Ya verifiqué
                      </button>
                    </div>
                  </div>
              ...
              <div className="space-y-6">
                <div>
                  <h3 className="text-lg font-bold text-white">Acceso de taller</h3>
                  <p className="text-sm text-slate-400 mt-1">Ingresa con el usuario entregado por administración.</p>
                </div>
                <form onSubmit={handleWorkshopAuth} className="space-y-4">
                  <div className="relative">
                    <LogIn className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-500" size={18} />
                    <input type="text" autoComplete="username" placeholder="Usuario" required value={username} onChange={(e) => setUsername(e.target.value)} disabled={isAuthLoading} className="w-full bg-slate-900/80 border border-slate-700 rounded-xl py-3 pl-12 pr-4 text-white focus:border-blue-500 focus:outline-none" />
                  </div>
                  <div className="relative">
                    <Lock className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-500" size={18} />
                    <input type="password" autoComplete="current-password" placeholder="Contraseña" required value={password} onChange={(e) => setPassword(e.target.value)} disabled={isAuthLoading} className="w-full bg-slate-900/80 border border-slate-700 rounded-xl py-3 pl-12 pr-4 text-white focus:border-blue-500 focus:outline-none" />
                  </div>
                  {authError && <p className="text-red-400 text-xs px-1">{authError}</p>}
                  <button type="submit" disabled={isAuthLoading} className="w-full py-3 bg-blue-600 hover:bg-blue-700 text-white rounded-xl font-bold disabled:opacity-50">
                    {isAuthLoading ? 'Ingresando…' : 'Ingresar al taller'}
                  </button>
                </form>
                <div className="relative flex items-center py-2">
                  <div className="flex-grow border-t border-slate-700"></div>
                  <span className="mx-4 text-slate-500 text-xs uppercase font-bold">Acceso de respaldo</span>
                  <div className="flex-grow border-t border-slate-700"></div>
                </div>
                <button onClick={() => handleSignIn()} disabled={isAuthLoading} className="w-full py-3 bg-white hover:bg-slate-100 text-slate-900 rounded-xl font-bold transition-all active:scale-95 flex items-center justify-center gap-3 disabled:opacity-75 disabled:cursor-not-allowed">
                  {isAuthLoading ? (
                ...
                <form onSubmit={handleEmailAuth} className="space-y-4">
                  <div className="relative">
                    ...
                    <input type="email" placeholder="Email" required value={email} onChange={(e) => setEmail(e.target.value)} disabled={isAuthLoading} className="w-full bg-slate-900/80 border border-slate-700 rounded-xl py-3 pl-12 pr-4 text-white focus:border-blue-500 focus:outline-none disabled:opacity-50" />
                  </div>
                  <div className="relative">
                    <Lock className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-500" size={18} />
                    <input type="password" placeholder="Contraseña" required value={password} onChange={(e) => setPassword(e.target.value)} disabled={isAuthLoading} className="w-full bg-slate-900/80 border border-slate-700 rounded-xl py-3 pl-12 pr-4 text-white focus:border-blue-500 focus:outline-none disabled:opacity-50" />
                  </div>
                  {authError && <p className="text-red-400 text-xs px-1">{authError}</p>}
                  {authNotice && <p className="text-emerald-400 text-xs px-1">{authNotice}</p>}
                  <button type="submit" disabled={isAuthLoading} className="w-full py-3 bg-blue-600 hover:bg-blue-700 text-white rounded-xl font-bold transition-all active:scale-95 disabled:opacity-50 disabled:cursor-not-allowed">
                    {isAuthLoading ? (
                ...
                <button type="button" onClick={handlePasswordReset} disabled={isAuthLoading} className="w-full text-sm text-blue-400 hover:text-blue-300 disabled:opacity-50">
                  ¿Olvidaste tu contraseña de respaldo? Recuperarla
                </button>
              </div>
            )}
          </div>
            ...
            <div className="bg-slate-800 p-6 rounded-2xl border border-blue-500/30 shadow-lg">
              <div className="flex items-start justify-between gap-4">
                <div>
                  ...
                  <p className="text-sm text-slate-400 mt-1">Estado: {getEffectiveAccountStatus(account) === 'expired' ? 'Vencido · modo lectura' : getEffectiveAccountStatus(account) === 'active' ? 'Activo' : 'En prueba'}</p>
                </div>
                <span className={`px-3 py-1 rounded-full text-xs font-bold ${canWrite ? 'bg-emerald-500/15 text-emerald-400' : 'bg-amber-500/15 text-amber-300'}`}>{canWrite ? 'Habilitado' : 'Renovación pendiente'}</span>
              </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mt-4">
                <button onClick={() => window.alert('El WhatsApp de soporte se incorporará antes de abrir el piloto comercial.')} className="py-3 rounded-xl bg-blue-600 hover:bg-blue-700 text-white font-bold">
                  {canWrite ? 'Contactar soporte' : 'Renovar Plan Fundador'}
                </button>
                <button onClick={() => setActiveSection('data')} className="py-3 rounded-xl bg-slate-900 border border-slate-700 text-slate-200 font-bold">
                  Respaldar mis datos
                </button>
              </div>
            </div>
          ...
          <button onClick={() => setActiveSection('legal')} className="group bg-slate-800 hover:bg-slate-750 p-6 rounded-2xl border border-slate-700 hover:border-emerald-500/50 transition-all shadow-lg flex items-center justify-between">
            <div className="flex items-center gap-4">
              ...
              <div className="text-left">
                <h3 className="text-lg font-bold text-white">Privacidad y condiciones</h3>
                <p className="text-sm text-slate-400">Consulta cómo protegemos y utilizamos los datos.</p>
              </div>
            </div>
          ...
          <button onClick={() => setActiveSection('company')} className="group bg-slate-800 hover:bg-slate-750 p-6 rounded-2xl border border-slate-700 hover:border-blue-500/50 transition-all shadow-lg flex items-center justify-between">
            <div className="flex items-center gap-4">
          ...
          <button onClick={() => setActiveSection('templates')} className="group bg-slate-800 hover:bg-slate-750 p-6 rounded-2xl border border-slate-700 hover:border-green-500/50 transition-all shadow-lg flex items-center justify-between">
            <div className="flex items-center gap-4">
          ...
          {/* Tarjeta 3: Tema */}
          <button onClick={() => setActiveSection('theme')} className="group bg-slate-800 hover:bg-slate-750 p-6 rounded-2xl border border-slate-700 hover:border-purple-500/50 transition-all shadow-lg flex items-center justify-between">
            <div className="flex items-center gap-4">
          ...
          <button onClick={() => setActiveSection('data')} className="group bg-slate-800 hover:bg-slate-750 p-6 rounded-2xl border border-slate-700 hover:border-yellow-500/50 transition-all shadow-lg flex items-center justify-between">
            <div className="flex items-center gap-4">
    ...
    return (
      <div className="space-y-6 pb-20 animate-fade-in">
        ...
        <div className="bg-slate-800 rounded-2xl border border-slate-700 p-6 shadow-lg">
          ...
          <p className="text-slate-400 text-sm mb-5">La aplicación conserva una copia local para trabajar sin conexión. Cuando inicias sesión, también sincroniza la información con tu cuenta en la nube.</p>
          ...
          <div className="mb-6">
            ...
            <div className="w-full bg-slate-900 h-3.5 rounded-full overflow-hidden border border-slate-750 p-0.5">
              <div className={`h-full rounded-full transition-all duration-500 ${stats.percent > 85 ? 'bg-red-500' : stats.percent > 50 ? 'bg-orange-500' : 'bg-blue-500'}`} style={{ width: `${stats.percent}%` }} />
            </div>
            {stats.percent > 80 && (
            ...
            <div className="space-y-2.5">
              <div className="flex justify-between items-center text-xs">
                ...
                <span className="text-slate-200 font-mono font-medium">
                  {(stats.services / 1024).toFixed(3)} KB ({services.length} registros)
                </span>
              </div>
              <div className="flex justify-between items-center text-xs">
                <span className="text-slate-400 flex items-center gap-1.5">
                ...
                <span className="text-slate-200 font-mono font-medium">
                  {(stats.costs / 1024).toFixed(3)} KB ({costs.length} registros)
                </span>
              </div>
              <div className="flex justify-between items-center text-xs">
                <span className="text-slate-400 flex items-center gap-1.5">
                ...
                <span className="text-slate-200 font-mono font-medium">
                  {(stats.quotes / 1024).toFixed(3)} KB ({quotes.length} registros)
                </span>
              </div>
              ...
              <div className="flex justify-between items-center text-xs">
                ...
                <span className="text-slate-200 font-mono font-medium font-bold">{settings.logoUrl ? `${(stats.logo / 1024).toFixed(1)} KB` : 'No configurado (0 KB)'}</span>
              </div>
              <div className="flex justify-between items-center text-xs">
                <span className="text-slate-400 flex items-center gap-1.5">
        ...
        <div className="bg-slate-800 rounded-2xl border border-slate-700 p-6 shadow-lg">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <button onClick={exportFullBackupJSON} className="px-5 py-4 bg-emerald-950/30 border border-emerald-500/30 hover:border-emerald-550 hover:bg-emerald-900/15 text-emerald-400 hover:text-white rounded-xl text-sm font-bold transition-all flex flex-col items-center gap-2 justify-center shadow-md active:scale-95 group">
            ...
            <button onClick={() => fullBackupInputRef.current?.click()} className="px-5 py-4 bg-blue-950/30 border border-blue-500/30 hover:border-blue-550 hover:bg-blue-900/15 text-blue-400 hover:text-white rounded-xl text-sm font-bold transition-all flex flex-col items-center gap-2 justify-center shadow-md active:scale-95 group">
        ...
        <div className="bg-slate-800 rounded-2xl border border-slate-700 p-6 shadow-lg">
          ...
          <p className="text-slate-400 text-sm mb-6">Descarga individual de tablas en formato Excel (CSV) o importar de forma granular por cada módulo. Muy útil para análisis externo.</p>

          <div className="space-y-4">
            {/* Servicios */}
            <div className="flex gap-2">
              <button onClick={exportServices} className="flex-1 flex items-center justify-between p-4 bg-slate-900 border border-slate-650 rounded-xl hover:bg-slate-700 hover:border-blue-500/50 transition-all group">
              ...
              <button onClick={() => serviceInputRef.current?.click()} className="px-4 bg-slate-900 border border-slate-650 rounded-xl hover:bg-slate-700 hover:border-blue-500/50 transition-all group flex flex-col items-center justify-center gap-1" title="Subir Archivo de Servicios">
            ...
            {/* Costos */}
            <div className="flex gap-2">
              <button onClick={exportCosts} className="flex-1 flex items-center justify-between p-4 bg-slate-900 border border-slate-650 rounded-xl hover:bg-slate-700 hover:border-red-500/50 transition-all group">
                <div className="flex items-center gap-3">
              ...
              <button onClick={() => costInputRef.current?.click()} className="px-4 bg-slate-900 border border-slate-650 rounded-xl hover:bg-slate-700 hover:border-red-500/50 transition-all group flex flex-col items-center justify-center gap-1" title="Subir Archivo de Costos">
            ...
            {/* Cotizaciones */}
            <div className="flex gap-2">
              <button onClick={exportQuotes} className="flex-1 flex items-center justify-between p-4 bg-slate-900 border border-slate-650 rounded-xl hover:bg-slate-700 hover:border-green-500/50 transition-all group">
                <div className="flex items-center gap-3">
              ...
              <button onClick={() => quoteInputRef.current?.click()} className="px-4 bg-slate-900 border border-slate-650 rounded-xl hover:bg-slate-700 hover:border-green-500/50 transition-all group flex flex-col items-center justify-center gap-1" title="Subir Archivo de Cotizaciones">
        ...
        <div className="p-5 bg-red-950/20 border border-red-500/35 rounded-2xl">
          ...
          {resetConfirmStep === 0 && (
            <>
              <p className="text-slate-450 text-xs mb-4 leading-relaxed">Esta acción borrará permanentemente de tu navegador toda la información del taller, incluyendo todas las órdenes de servicios, gastos, presupuestos y configuraciones de marca. Dejará la app 100% nueva y vacía.</p>
              <button
                onClick={() => {
          ...
          {resetConfirmStep === 1 && (
            <div className="space-y-3 bg-red-950/40 p-4 rounded-xl border border-red-500/20 animate-fade-in">
              ...
              <p className="text-slate-200 text-xs leading-relaxed">¿Estás completamente seguro de que deseas eliminar TODOS los datos de la aplicación? Se borrarán de forma inmediata y definitiva:</p>
              <ul className="text-slate-300 text-xs space-y-1 list-disc list-inside bg-slate-900/50 p-2.5 rounded-lg border border-slate-800">
                <li>
                  Órdenes de Servicio: <span className="font-bold text-red-400 font-mono">{services.length}</span> registros
                </li>
                <li>
                  Módulo de Gastos: <span className="font-bold text-red-400 font-mono">{costs.length}</span> registros
                </li>
                <li>
                  Cotizaciones/Presupuestos: <span className="font-bold text-red-400 font-mono">{quotes.length}</span> registros
                </li>
                <li>Color de tema, nombre, dirección, teléfono y logotipo de la empresa</li>
              </ul>
              <p className="text-orange-400 font-bold text-[11px]">🚨 Esta acción NO se puede deshacer. Se recomienda descargar un Respaldo Completo (.JSON) de seguridad más arriba si deseas conservar tu información.</p>
              <div className="grid grid-cols-2 gap-3.5 pt-1.5">
                <button type="button" onClick={() => setResetConfirmStep(0)} className="py-2.5 bg-slate-900 hover:bg-slate-800 text-slate-300 border border-slate-700 font-bold rounded-lg text-xs transition-colors">
                  Cancelar / Volver
                </button>
                <button type="button" onClick={() => setResetConfirmStep(2)} className="py-2.5 bg-red-800 hover:bg-red-700 text-white font-extrabold rounded-lg text-xs transition-colors">
          ...
          {resetConfirmStep === 2 && (
            <div className="space-y-3 bg-red-950/40 p-4 rounded-xl border border-red-500/20 animate-fade-in">
              ...
              <input type="text" placeholder="Escribe BORRAR aquí" value={resetInputWord} onChange={(e) => setResetInputWord(e.target.value)} className="w-full bg-slate-900 border border-red-500/40 rounded-lg py-2.5 px-4 text-sm text-center font-black text-red-400 focus:border-red-500 focus:outline-none tracking-widest uppercase placeholder-slate-600 focus:ring-1 focus:ring-red-500" />
              ...
              <div className="grid grid-cols-2 gap-3.5 pt-1.5">
                ...
                <button
                  ...
                  onClick={() => {
                    ...
                    ['taller_services', 'taller_costs', 'taller_quotes', 'taller_settings', 'service_draft'].forEach((key) => localStorage.removeItem(key));
        ...
        {user && (
          <div className="p-5 bg-slate-900 border border-red-500/35 rounded-2xl">
            <h3 className="text-red-400 font-extrabold text-sm flex items-center gap-2">
    ...
    return (
      <div className="space-y-6 pb-20 animate-fade-in">
        ...
        <div className="bg-slate-800 rounded-2xl border border-slate-700 p-6 shadow-lg space-y-6">
          <section>
            <h2 className="text-xl font-bold text-white mb-3">Política de privacidad</h2>
            <div className="space-y-3 text-sm text-slate-300 leading-relaxed">
              <p>Gestión Taller almacena los datos de la cuenta, configuración del taller, clientes, vehículos, servicios, cotizaciones, gastos y pagos que el usuario registra. Estos datos se utilizan para entregar el servicio, sincronizar la información y prestar soporte.</p>
              <p>Cada cuenta puede acceder únicamente a la información de su propio taller. El usuario puede descargar un respaldo y eliminar su cuenta y datos desde Configuración. Gestión Taller no vende la información registrada.</p>
              <p>El taller es responsable de contar con autorización para registrar información de sus clientes. Las fotografías en la nube se mantienen desactivadas durante esta etapa.</p>
            </div>
          </section>
          <section className="border-t border-slate-700 pt-6">
            <h2 className="text-xl font-bold text-white mb-3">Términos del servicio</h2>
            <div className="space-y-3 text-sm text-slate-300 leading-relaxed">
              <p>La prueba gratuita general dura 15 días. Los primeros 10 talleres colaboradores podrán recibir 30 días a cambio de utilizar la aplicación y entregar comentarios reales.</p>
              <p>Al finalizar la prueba, la cuenta pasa a modo lectura y conserva sus datos. El Plan Fundador tiene un precio inicial de $10.000 CLP mensuales y se activa manualmente después de confirmar el pago.</p>
              <p>Gestión Taller es una herramienta de administración y no sustituye sistemas contables, tributarios ni documentos legales obligatorios. El responsable comercial y el canal de contacto definitivo se incorporarán antes del lanzamiento.</p>
            </div>
          </section>
        </div>
      </div>
      ...
      <div className="space-y-6 pb-20 animate-fade-in">
        ...
        <div className="bg-slate-800 rounded-2xl border border-slate-700 p-6 shadow-lg">
          <h2 className="text-xl font-bold text-white mb-6 flex items-center gap-2">
            <Building2 className="text-blue-500" /> Datos del Taller
          </h2>
            ...
            <div>
              ...
              <div className="flex items-start gap-4">
                ...
                <div className="flex flex-col gap-2">
                  ...
                  <button onClick={() => logoInputRef.current?.click()} className="px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded-lg text-sm font-medium transition-colors flex items-center gap-2">
                    <Upload size={16} /> Subir Imagen
                  </button>
                  {settings.logoUrl && (
                    <div className="flex flex-col gap-1">
                      <button onClick={removeLogo} className="px-4 py-2 bg-slate-900 hover:bg-red-900/30 text-red-400 hover:text-red-300 border border-slate-700 hover:border-red-500/30 rounded-lg text-sm font-medium transition-colors flex items-center gap-2 self-start">
                      ...
                      <div className="text-[11px] bg-slate-900/60 p-2.5 rounded-xl border border-slate-700/50 max-w-[240px]">
                        ...
                        <span className="text-blue-400 font-bold font-mono">{(getLogoOriginalSize() / 1024).toFixed(1)} KB</span>
                        <span className="text-[9px] text-slate-500 block mt-0.5">(Equivale a ~{((getLogoOriginalSize() * 2.66) / 1024).toFixed(1)} KB de almacenamiento local)</span>
                        {getLogoOriginalSize() > 200 * 1024 && <div className="mt-1 text-orange-400 text-[10px] font-bold">⚠️ Imagen muy pesada. Recomendamos menos de 150 KB para no saturar tu espacio.</div>}
                      </div>
                    </div>
                  )}
  ...
  if (activeSection === 'theme') {
    return (
      <div className="space-y-6 pb-20 animate-fade-in">
        ...
        <div className="bg-slate-800 rounded-2xl border border-slate-700 p-6 shadow-lg">
          <h2 className="text-xl font-bold text-white mb-6 flex items-center gap-2">
            <Palette className="text-purple-500" /> Color del Sistema
          </h2>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            {themes.map((theme) => (
              <button key={theme.id} onClick={() => handleChange('themeColor', theme.id)} className={`p-4 rounded-xl border flex items-center gap-4 transition-all ${settings.themeColor === theme.id ? 'border-white bg-slate-700 shadow-md transform scale-105' : 'border-slate-700 bg-slate-900 hover:bg-slate-800'}`}>
                <div className={`w-12 h-12 rounded-full ${theme.color} shadow-lg border-2 border-slate-800 flex items-center justify-center`}>{settings.themeColor === theme.id && <Check className="text-white" size={20} />}</div>
        ...
        {editingTemplate && (
          <div className="fixed inset-0 z-[9999] bg-slate-950 flex flex-col h-[100dvh] w-full animate-in slide-in-from-bottom duration-300">
            ...
            <div className="px-4 py-3 border-b border-slate-800 bg-slate-900/95 backdrop-blur-md flex justify-between items-center shrink-0 shadow-lg z-50">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <h3 className="text-base font-bold text-white leading-none">{editingTemplate === 'service' ? 'Servicio / Orden' : 'Cotización'}</h3>
                </div>
              </div>
              <button onClick={saveEditor} className="px-5 py-2 rounded-xl bg-blue-600 hover:bg-blue-700 text-white font-bold text-sm flex items-center gap-2 shadow-lg shadow-blue-900/20 active:scale-95 transition-all">
            ...
            <div className="flex-1 bg-slate-950 relative">
              <textarea autoFocus value={tempTemplateValue} onChange={(e) => setTempTemplateValue(e.target.value)} className="w-full h-full bg-slate-950 text-white font-mono text-base sm:text-lg p-6 focus:outline-none resize-none leading-relaxed" placeholder="Escribe tu mensaje aquí..." />
            </div>
